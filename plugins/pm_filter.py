        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                if clicked == typed:
                    ...
                    await query.answer(f"𝖧𝖾𝗒 {query.from_user.first_name}, 𝖳𝗁𝗂𝗌 𝗂𝗌 𝗇𝗈𝗍 𝗒𝗈𝗎𝗋 𝗋𝖾𝗊𝗎𝖾𝗌𝗍 !", show_alert=True)
                await query.answer('𝖢𝗁𝖾𝖼𝗄 𝖯𝖬, 𝖨 𝗁𝖺𝗏𝖾 𝗌𝖾𝗇𝗍 𝖿𝗂𝗅𝖾𝗌 𝗂𝗇 𝖯𝖬\n(تــم ارســاله عـلـي الـخــاص)', show_alert=True)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"hello", callback_data=f'{pre}#{file.file_id}'
                ),
            ]
            for file in files
        ]
    ...
    if imdb and imdb.get('poster'):
        ...
        if message.chat.id == SUPPORT_CHAT_ID:
            await message.reply_text(f"<b>👋 𝖧𝖾𝗒 {message.from_user.mention} \n📁 {str(total_results)} 𝖱𝖾𝗌𝗎𝗅𝗍𝗌 𝖺𝗋𝖾 𝖿𝗈𝗎𝗇𝖽 𝖿𝗈𝗋 𝗒𝗈𝗎𝗋 𝗊𝗎𝖾𝗋𝗒 {search}.\n\nKindly ask movies and series here ⬇\@ArrowFlix </b>")
        else:
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            if btn is not None:
                try:
                    if fileid == "None":
                        if btn == "[]":
                            arrowflix = await client.send_message(
                                group_id, 
                                reply_text, 
                                disable_web_page_preview=True,
                        ...
                        arrowflix = await client.send_cached_media(
                            group_id,
                            fileid,
                        ...
                        button = eval(btn)
                        arrowflix = await message.reply_cached_media(
                            fileid,
                            caption=reply_text or "",
                            reply_markup=InlineKeyboardMarkup(button),
                            reply_to_message_id=reply_id
